...
function __fish_granted_no_subcommand --description 'Test if there has been any subcommand yet'
    for i in (commandline -opc)
        if contains -- $i browser set set-sso settings print profile-order set export-suffix set request-url set clear set completion help h token sso-tokens list clear expiry uninstall sso generate populate login credentials add import update list remove export-plaintext rotate import-from-env credential-process registry add sync remove migrate setup console cache clear list doctor help h
            return 1
        end
    end
    return 0
...
complete -c granted -n '__fish_seen_subcommand_from generate' -f -l source -r -d 'The sources to load AWS profiles from (valid values are: \'aws-sso\')'
...
complete -c granted -n '__fish_seen_subcommand_from generate' -f -l sso-browser-profile -r -d 'Use a pre-existing profile in your browser for SSO login'
complete -c granted -n '__fish_seen_subcommand_from generate' -f -l use-device-code -d 'Force device code flow even if authorization code with PKCE is enabled'
...
complete -c granted -n '__fish_seen_subcommand_from populate' -f -l sso-browser-profile -r -d 'Use a pre-existing profile in your browser for SSO login'
complete -c granted -n '__fish_seen_subcommand_from populate' -f -l use-device-code -d 'Force device code flow even if authorization code with PKCE is enabled'
...
complete -c granted -n '__fish_seen_subcommand_from login' -f -l sso-browser-profile -r -d 'Use a pre-existing profile in your browser for SSO login'
complete -c granted -n '__fish_seen_subcommand_from login' -f -l use-device-code -d 'Force device code flow even if authorization code with PKCE is enabled'
complete -c granted -n '__fish_seen_subcommand_from login' -f -l use-authorization-code -d 'Use authorization code flow with PKCE instead of device code'
...
complete -c granted -n '__fish_seen_subcommand_from add' -f -l ref -r -d 'Git ref (branch, tag, or commit) to checkout'
...
complete -c assume -n __fish_assume_no_subcommand -f -l sso-browser-profile -r -d 'Use a pre-existing profile in your browser for SSO login'
...
complete -c assume -n __fish_assume_no_subcommand -f -l use-authorization-code -d 'Use authorization code flow with PKCE for SSO login'
